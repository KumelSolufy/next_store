...
def validate(self,method):
	if self.is_pos and self.pos_profile and self.set_warehouse:
		if not frappe.db.get_value("POS Profile",self.pos_profile,"warehouse") == self.set_warehouse:
			frappe.throw("Warehouse '{0}' is not allocated for POS Profile '{1}'".format(self.set_warehouse,self.pos_profile))
